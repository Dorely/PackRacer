...
import { calculateStandings, type Heat, type Race, type RaceEvent, type Standing } from '@packracer/race-engine'
...
function RaceSpecificDisplay({ context }: { context: DisplayContext }) {
  switch (context.race.format) {
    case 'single-elimination':
      return <SingleEliminationDisplay context={context} />
    case 'round-robin':
      return <RoundRobinDisplay context={context} />
    case 'points-heats':
      return <HeatDisplay context={context} label="Current Heat" />
    case 'timed-heats':
    default:
      return <HeatDisplay context={context} label="Current Heat" />
  }
...
export function DisplayMode({ event, currentRace, selectedRaceId, setSelectedRaceId }: SectionProps) {
  ...
  return (
    <section className="display-board" data-format={currentRace.format}>
      ...
      <RaceSpecificDisplay context={context} />
    </section>
  )
}
